...
Sois naturel et utile."""
    ...
    async def process_message_stream(
        ...
        """
        🚀 Process message with streaming support (SSE)
        
        Yields chunks as they're generated by the LLM
        Provides instant feedback to the user
        
        Yields:
            dict: Events with type 'chunk' or 'done'
        """
        import time
        start_time = time.time()
        function_called = None
        ...
        try:
            logger.info(f"[Stream] Processing message from {user_id} ({user_role_normalized})")
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                async with client.stream(
                    ...
                    if response.status_code != 200:
                        ...
                        yield {
                            "type": "error",
                            "message": "Erreur lors de l'appel au LLM"
                        }
                        return
                    
                    # Parser le stream SSE de Groq
                        ...
                        if line.startswith("data: "):
                            ...
                            try:
                                ...
                                if "function_call" in delta:
                                    # Le LLM veut appeler une fonction
                                    # On ne stream pas les function calls, on les exécute
                                ...
                                # Streamer le contenu textuel
                                if "content" in delta and delta["content"]:
                                    chunk_text = delta["content"]
                                    full_response += chunk_text
                                    
                                    # Nettoyer les balises techniques en temps réel
                                    cleaned_chunk = self._clean_technical_tags(chunk_text)
                                    if cleaned_chunk:
                                        yield {
                                            "type": "chunk",
            ...
            # Nettoyer la réponse complète
            full_response = self._clean_technical_tags(full_response)
            
            # Générer les suggestions
            suggestions = await self._generate_suggestions(message, user_role_normalized, full_response)
